...
import { listPartnerBds, listPartnerNetwork, listPartnerRolesPortal, partnerBdMeta, listAttachableEmployees, listPendingPartners, partnerUsers } from "@/lib/portal-db";
...
export default async function NetworkPage() {
  ...
  if (s.role === "owner") {
    let bds: any[] = [], roles: string[] = [], pending: any[] = [], error: string | null = null;
    try {
    ...
    return <NetworkDashboard mode="owner" name={s.name || "Owner"} bds={bds} roles={roles} pending={pending} error={error} />;
  }

  let isBd = false, network: any[] = [], roles: string[] = [], attachable: any[] = [], users: any[] = [], error: string | null = null;
  try {
